...
export default function ReceiveShipmentPage({ params }: { params: { shipmentId: string } }) {
  ...
  const loadShipment = async () => {
    try {
      ...
      // Initialize received items with dispatched quantities
      const initialReceivedItems = found.items.map((item: any) => ({
  ...
  const handleReceive = async () => {
    ...
    try {
      ...
      router.push(`/distributor/shipments/${shipment.id}`)
    } catch (err) {
