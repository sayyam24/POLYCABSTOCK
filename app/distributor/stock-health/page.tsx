...
import { AlertTriangle, Package, FileText, Copy, Truck, RotateCcw, ArrowRight, RefreshCw, TrendingDown, AlertCircle, CheckCircle, XCircle } from 'lucide-react'
...
export default function StockHealthPage() {
  ...
  useEffect(() => {
    ...
    if (autoRefresh) {
      const interval = setInterval(loadHealthData, 60000) // Refresh every 60 seconds
  ...
  const loadHealthData = async () => {
    try {
      ...
      // Duplicate Invoices (placeholder - would need actual duplicate detection logic)
      setDuplicateInvoices([])

      // Unmatched Products (placeholder - would need actual unmatched product logic)
  ...
  return (
    <DashboardLayout role="distributor">
      ...
      <main className="p-4 lg:p-8">
                ...
                {negativeStock.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-200">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => router.push('/distributor/ledger')}
                    >
                      View Ledger
                    </Button>
                  </div>
        ...
        {/* Duplicate Invoice Warning Widget */}
              ...
              <div className="space-y-2">
                {pendingShipments.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    <Button
                      size="sm"
                      onClick={() => router.push(`/distributor/shipments/${item.shipment.id}`)}
                    >
                      View
                    </Button>
                  </div>
        ...
        {/* Recent Stock Adjustments Widget */}
